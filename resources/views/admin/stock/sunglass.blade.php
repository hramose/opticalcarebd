...
@section('content')
        ...
        <div id="page_content_inner">
            <h4 class="heading_a uk-margin-bottom">Stock Sunglass</h4>
                       <div class="uk-width-medium-1-6">
                <div class="uk-width-medium-1-3">
                            <button id="addSunglassModel" class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light" data-uk-modal="{target:'#model_sunglass'}">Add&nbsp;New&nbsp;Sunglass</button>
                            <div class="uk-modal" id="model_sunglass">
                                <div class="uk-modal-dialog">
                                   ...
                                   <form id="form_validation" class="uk-form-stacked" method="POST">
                                        {{ csrf_field() }}
                                    {{ method_field('DELETE') }} 
                                    <input type="hidden" id="sunglassId">
...
@section('jslink')
    ...
    <script>
        $(document).ready(function(){
            ...
            var table  = $('#dt_tableExport').DataTable({
                ...
                "columns":[
                    ...
                    return '<a id="updateQuantityModel" data-uk-modal="{target:'+"'#model_sunglass'"+'}"><i class="md-icon material-icons md-36 uk-text-primary" data-id="' + row.id + '">&#xE923;</i><input type="hidden" id="sunglassCategory" value="'+ row.category +'"><input type="hidden" id="sunglassQuantity" value="'+ row.quantity +'"></a> <a id="editModel" data-uk-modal="{target:'+"'#model_sunglass'"+'}"><i class="md-icon material-icons md-36 uk-text-primary" data-id="' + row.id + '">&#xE254;</i><input type="hidden" id="editSunglassCategory" value="'+ row.category +'"><input type="hidden" id="editSunglassQuantity" value="'+ row.quantity +'"></a>  <a id="deleteSunglass"><i class="md-icon material-icons md-36 uk-text-danger" data-id="' + row.id + '">&#xE92B;</i></a>'
                }
                }
                ]
            });
            ...
            $(document).on('click','#addSunglassModel', function(event){
            $('.uk-modal-title').text('Add New Sunglass');
             $("#txtCategory").val("");
            $('#txtQuantity').val("");
        ...
        $(document).on('click','#updateQuantityModel', function(event){
            ...
            var  category = $(this).find("#sunglassCategory").val();
            var  quantity = $(this).find("#sunglassQuantity").val();
            $("#sunglassId").val(id);
         ...
         $(document).on('click', '#editModel', function(event){
            var id = $(this).find( "i" ).attr( "data-id" ); 
            var category = $(this).find('#editSunglassCategory').val();
            var quantity = $(this).find('#editSunglassQuantity').val();
            $('#sunglassId').val(id);
            $("#txtCategory").val(category);
            $('#txtQuantity').val(quantity);
            $('.uk-modal-title').text('Edit Sunglass Info');
            $("#txtCategory").prop('disabled', false);
            $("#txtQuantity").prop('disabled', false);                    
            $('#btnEdit').show();
            $('#btnAddSunglass').hide(0);
            $('#btnUpdateQuantity').hide(0);
            $('#addQuantity').hide(0);
        });
        $('#btnAddSunglass').click(function(event){
            $.ajax({
                type: 'POST',
                url: 'sunglass',
                data: {
                ...
                success: function(data) {
                     if ((data.errors)) {
                        ...
                        table.ajax.reload( null, false);
                         swal("Good job!", "New Sunglass Successfully Added!", "success")
                    }
                },
            });
        });
        $(document).on('click','#btnUpdateQuantity', function(event){
            var id = $( "#sunglassId" ).val();
            ...
            $.ajax({
                ...
                success: function(data) {
                     if ((data.errors)) {
                        ...
                        table.ajax.reload( null, false);
                         swal("Good job!", "Sunglass Quantity Successfully Updated!", "success")
                    }
                },
            });
        });
            ...
            $.ajax({
                ...
                success: function(data) {
                     if ((data.errors)) {
                        ...
                        table.ajax.reload( null, false);
                         swal("Good job!", "Sunglass info Successfully Updated!", "success")
                    }
                },
            });
        });
        ...
        swal({
                ...
                function(isConfirm){
                if (isConfirm) {
                     $.post('sunglass/delete', {'id': id, '_token':$('input[name=_token]').val(),'_method':$('input[name=_method]').val()}, function(data){
                        table.ajax.reload( null, false );                    
                    });
